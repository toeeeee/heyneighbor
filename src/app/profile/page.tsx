    ...
    useEffect(() => {
        ...
        const getTradesAsRequestee = async () => {
            const response = await fetch(
                `/api/user/trade/${userId}?who=requestee`
            );
            const tradeData: { trades: TradeType[] } | null =
                await response.json();
            if (!tradeData) return;
                ...
                <Card className="border-4 border-gray-700 bg-gray-900">
                    ...
                    <CardContent className="p-6">
                        <div className="flex items-center space-x-4">
                            ...
                            <div className="w-24 h-24 bg-gray-700 border-4 border-gray-600">
                               {user && <img src={user.image_url} alt='pfp'></img> }
                            </div>
                            <div>
                                ...
                                <div className="flex items-center justify-between">
                                    <div>
                                        <p className="text-white">
                                            Offering:{" "}
                                            {trade.requestorItemId.itemName}
                                        </p>
                                        <p className="text-white">
                                            For:{" "}
                                            {trade.requesteeItemId.itemName}
                                        </p>
                                    </div>
